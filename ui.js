...
(function() {
  ...
  function createFirkiContainer() {
    ...
    closeBtn.addEventListener('click', toggleFirkiPanel);
  ...
  async function generateDynamicContent() {
    try {
      ...
      // Use the existing boolean search module
      let booleanSearch = '';
      console.log('🔍 BooleanSearch module available:', !!window.BooleanSearch);
      console.log('🔍 BooleanSearch.generateBooleanSearch available:', !!(window.BooleanSearch && window.BooleanSearch.generateBooleanSearch));
      
      if (window.BooleanSearch && window.BooleanSearch.generateBooleanSearch) {
        console.log('🔍 Using BooleanSearch module...');
        try {
          booleanSearch = await window.BooleanSearch.generateBooleanSearch(parsedJD, 'simple');
          console.log('🔍 BooleanSearch module result:', booleanSearch);
        } catch (error) {
          console.error('❌ BooleanSearch module error:', error);
          booleanSearch = generateFallbackBooleanSearch(parsedJD);
        }
      } else {
        console.log('⚠️ BooleanSearch module not available, using fallback');
        booleanSearch = generateFallbackBooleanSearch(parsedJD);
      }
      ...
      console.log('🔍 Final boolean search:', booleanSearch);
      
      // Update the UI with dynamic content and Gemini context
      updateUIWithDynamicContent(skills, booleanSearch, geminiContext);
  ...
  function setupMutationObserver() {
    const observer = new MutationObserver(() => {
      if (isInEmailView()) {
        ...
        // If we are not in an email view (e.g., inbox), remove the container.
